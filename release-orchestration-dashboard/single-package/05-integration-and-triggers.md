...
- *Deployment requirement:* the process that runs the trigger (worker) must have the
  Playwright browser installed. In the **single-exe** build this is a packaging decision
  (`06`): bundle the browser into the exe (large), download on first run, or exclude
  Playwright entirely if browser triggers aren't needed locally.
...
- **Encrypt at rest; never log; scope per user.** (Data model: credentials belong to a
  user — Rule 3.)
- **Local desktop tool (`01` §7):** credentials are encrypted **on the developer's
  machine** using the **OS keystore** (Windows DPAPI / macOS Keychain) — **never** a key
  hardcoded in the exe (trivially extractable). Build the store behind an interface so the
  backing mechanism is swappable, but there is no `shared`/server-side store in scope.
...
- Storing **passwords on the developer's machine** is the weakest posture in the design.
  Mitigate with the **OS keystore** (never a hardcoded key), prefer `token` over passwords,
  and prefer **Tier 3 manual** (below) so no password is stored at all. Since there is no
  shared/server-side store in scope, this mitigation is the *only* line of defence — treat
  it as mandatory.
